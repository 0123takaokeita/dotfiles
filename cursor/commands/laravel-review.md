...
updated: 2026-03-01T11:57
---
# Laravel Linter 要約（/laravel-review）

このコマンドは **Laravel プロジェクト** でだけ実行すること。まず `composer.json` に `laravel/framework` が含まれるか確認し、Laravel プロジェクトでない場合は「このプロジェクトは Laravel ではありません」とだけ返す。
